...
class DestinationSearch extends Component {
  state = {
    searchInput: '',
    destList: destinationsList,
  }

  filteredDestinationList = event => {
    this.setState({
  ...
  render() {
    ...
    const destResults = destList.filter(eachDest =>
    ...
    return (
      <div>
        ...
        <input
          type="search"
          onChange={this.filteredDestinationList}
          value={searchInput}
        />
        <label>
          <img
        ...
        <ul>
          {destResults.map(eachDest => (
            <DestinationItem destDetails={eachDest} key={eachDest.id} />
          ))}
        </ul>
      </div>
    )
